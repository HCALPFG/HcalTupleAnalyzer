...
hists = { key.GetName().split("_")[-1]: [inFile.Get("_".join(key.GetName().split("_")[:-2]+["plusPhi",key.GetName().split("_")[-1]] ) ),inFile.Get("_".join(key.GetName().split("_")[:-2]+["minusPhi",key.GetName().split("_")[-1]] ) )] for key in r.gDirectory.GetListOfKeys() }
...
for key,hists in hists.iteritems():
	...
	upperErrors.append(hists[0].GetStdDev(1))
	lowerMeanValues.append(hists[1].GetMean(1))
	lowerErrors.append(hists[1].GetStdDev(1))
...
upperTimingVsRunNumber.Draw("AP2")
c.cd(2)
...
lowerTimingVsRunNumber.Draw("AP2")
